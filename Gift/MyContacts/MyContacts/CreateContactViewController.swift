...
class CreateContactViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func showAlertMessage(msg: String) {
        let alert = UIAlertController(title: "Info", message: msg, preferredStyle: .actionSheet)
    ...
    func createAddressBookContactWithFirstName(_ firstName: String, lastName: String, email: String?, phone: String?, image: UIImage?) {
        ...
        if abAuthStatus == .denied || abAuthStatus == .restricted {
            self.showAlertMessage(msg: "Sorry, you are not authorize to access the contacts.")
            return
        }
        ...
        if !ABRecordSetValue(newContact, kABPersonFirstNameProperty, firstName as CFTypeRef, nil) {
            self.showAlertMessage(msg: "Error setting first name for the new contact")
            return
        }
        // last name
        if !ABRecordSetValue(newContact, kABPersonLastNameProperty, lastName as CFTypeRef, nil) {
            self.showAlertMessage(msg: "Error setting last name for the new contact")
            return
        }
        // email
        if email != nil {
            ...
            if !ABRecordSetValue(newContact, kABPersonEmailProperty, emails, nil) {
                self.showAlertMessage(msg: "Error setting email for the new contact")
                return
            }
        ...
        if phone != nil {
            ...
            if !ABRecordSetValue(newContact, kABPersonPhoneProperty, phoneNumbers, nil) {
                self.showAlertMessage(msg: "Error setting phone number for the new contact")
                return
            }
        }
        ...
        if image != nil {
            ...
            if !ABPersonSetImageData(newContact, imageData as CFData!, nil) {
                self.showAlertMessage(msg: "Error setting image for the new contact")
                return
            }
        }
        ...
        if errorSavingContact { self.showAlertMessage(msg: "There was an error storing your new contact. Please try again.") }
    ...
    func createCNContactWithFirstName(_ firstName: String, lastName: String, email: String?, phone: String?, image: UIImage?) {
        ...
        do {
            ...
            self.showAlertMessage(msg: "I was unable to create the new contact. An error occurred.")
        }
    }
        ...
        if let firstName = firstNameTextfield.text , firstName.characters.count > 0,
            ...
            if type == .addressBookContact {
                ...
                if #available(iOS 9, *) {
                    ...
                    self.showAlertMessage(msg: "Sorry, you can only use the Contacts framework from iOS 9.")
                }
                
            }
        } else {
            self.showAlertMessage(msg: "Please, insert at least a first and last name for the contact.")
        }
    }
